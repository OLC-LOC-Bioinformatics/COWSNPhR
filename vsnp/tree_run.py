...
class Tree(object):
    ...
    def main(self):
        """
        Run all the tree-specific methods
        """
        self.vcf_load()
    ...
    def vcf_load(self):
        ...
        file_list = TreeMethods.file_list(file_path=self.file_path)
        logging.info('Extracting sequence names')
        self.strain_vcf_dict = TreeMethods.strain_list(vcf_files=file_list)
        ...
        self.accession_species_dict = TreeMethods.parse_accession_species(ref_species_file=os.path.join(
        ...
        if self.variant_caller == 'deepvariant':
            self.strain_parsed_vcf_dict, self.strain_best_ref_dict, self.strain_best_ref_set_dict = \
                TreeMethods.load_gvcf(strain_vcf_dict=self.strain_vcf_dict,
    ...
    def load_snp_sequence(self):
        ...
        self.strain_species_dict, strain_best_ref_fasta_dict = \
            TreeMethods.determine_ref_species(strain_best_ref_dict=self.strain_best_ref_dict,
        ...
        defining_snp_dict = \
            TreeMethods.extract_defining_snps(reference_link_path_dict=self.reference_link_path_dict,
        ...
        self.strain_groups = TreeMethods.determine_groups(strain_snp_positions=strain_snp_positions,
        ...
        group_positions_set = \
            TreeMethods.determine_group_snp_positions(strain_snp_positions=strain_snp_positions,
                                                      strain_groups=self.strain_groups,
                                                      strain_species_dict=self.strain_species_dict)
        # Filter the positions if desired
        if self.filter_positions:
            group_positions_set = TreeMethods.filter_snps(group_positions_set)
        if self.debug:
        ...
        self.group_strain_snp_sequence, non_identical_group_positions = \
            TreeMethods.remove_identical_calls(group_strain_snp_sequence=group_strain_snp_sequence,
                                               consolidated_ref_snp_positions=consolidated_ref_snp_positions)
    ...
    def phylogenetic_trees(self):
        ...
        species_group_trees = TreeMethods.run_raxml(group_fasta_dict=self.group_fasta_dict,
                                                    strain_consolidated_ref_dict=self.strain_consolidated_ref_dict,
                                                    strain_groups=self.strain_groups,
                                                    threads=self.threads,
    ...
    def annotate_snps(self):
        ...
        full_best_ref_gbk_dict = \
            TreeMethods.load_genbank_file(reference_link_path_dict=self.reference_link_path_dict,
    ...
    def __init__(self, path, threads, debug, variant_caller, filter_positions):
        ...
        """
        logging.info('Phylogenetic tree creation module')
